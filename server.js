...
// ── GET /member/:slug — Private passport (redirect to passport demo) ───────
app.get('/member/:slug', (req, res) => {
  const slug   = req.params.slug;
  const name   = (req.query.name   || 'Member').trim();
  const number = (req.query.number || '001').trim().padStart(3, '0');
  const date   = (req.query.date   || '').trim();
  res.redirect(`/passport-demo.html?name=${encodeURIComponent(name)}&number=${encodeURIComponent(number)}&date=${encodeURIComponent(date)}&slug=${encodeURIComponent(slug)}&page=0`);
});
